...
@section('content')
    <section class="page-hero">
        <div class="max-w-3xl">
            ...
            <h2 class="headline mt-4">Cuéntanos lo que necesitas y te ayudamos con tus pedidos, consultas y celebraciones.</h2>
            <p class="subheadline mt-4">
                Estamos listos para atenderte con tortas personalizadas, pedidos especiales y cualquier consulta sobre la panadería.
            </p>
        </div>
    </section>
    ...
    <section class="section-space grid gap-6 lg:grid-cols-[1fr_0.9fr]">
        <div class="rounded-[2rem] border border-amber-200 bg-white/90 p-8 shadow-sm">
            <h3 class="section-title text-2xl">Escríbenos</h3>
            ...
            <form action="{{ route('web.contact.submit') }}" method="POST" class="mt-8 space-y-4">
                ...
                <div>
                    ...
                    <textarea id="mensaje" name="mensaje" rows="6" required class="input min-h-36" placeholder="Cuéntanos qué necesitas">{{ old('mensaje', request('mensaje')) }}</textarea>
                    @error('mensaje')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <div class="rounded-[2rem] border border-amber-200 bg-white/90 p-8 shadow-sm">
            <h3 class="section-title text-2xl">Información de contacto</h3>
            <div class="mt-5 space-y-3 text-sm leading-7 text-stone-700">
                <p><strong>Dirección:</strong> Jr. Parra del Riego #164, El Tambo, Huancayo</p>
                <p><strong>Teléfono:</strong> 993560096</p>
