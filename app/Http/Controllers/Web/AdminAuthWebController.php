...
class AdminWebController extends Controller
{
    ...
    private function renderExcelHtmlReport(string $title, string $accent, array $rows): string
    {
        ...
        $html .= '<p class="subtitle">Reporte generado el ' . now()->format('d/m/Y H:i') . '</p>';
        ...
        foreach ($rows as $row) {
            ...
            foreach ($headers as $index => $header) {
                $value = $row[$index] ?? '';
                $html .= '<td>' . htmlspecialchars($this->normalizeReportCell($value), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</td>';
            }
            $html .= '</tr>';
        }
